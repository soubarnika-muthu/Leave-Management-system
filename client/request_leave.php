	...
	if($result->num_rows > 0)
		{
			while($row = $result->fetch_assoc())
				...
				if($row['EarnLeave'] > 0)
					{
					echo "<button type = 'submit' name = 'type' value = 'Earn Leave' class = 'login-button shadow'>On-Duty</button>";	
					}
				else
					{
				...
				if($row['CasualLeave'] > 0)
					{
					echo "<button type = 'submit' name = 'type' value = 'Casual Leave' class = 'login-button shadow'>Others</button>";	
					}
				else
					{
